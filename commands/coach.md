...
## Context
- Active coach state: !`node scripts/coach-runner.mjs status --json 2>/dev/null || echo '{"active":false}'`

## Your task
Perform the requested Coach action now using the plugin's loaded `coach` skill and the provided arguments.

- Use `node scripts/coach-runner.mjs` for threshold resolution, state persistence, resume/status, Round 0 topology, and standard persistence.
