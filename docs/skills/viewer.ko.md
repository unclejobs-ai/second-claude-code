...
띄운 뷰어는 로컬이고 30분 놀면 죽습니다. 결과를 남에게 건네려면 내보내기를 씁니다.

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/export-artifact.mjs" --out pdca-export.md
```

마크다운 한 장을 쓰고 `{"out","artifacts","cycles","source"}`를 찍습니다. Artifact 도구로 발행하면 공유 URL이 나옵니다. 차트와 플로우는 mermaid로 바뀌므로 번들도, 깨질 외부 자산도 없습니다.
...
내보낸 페이지가 앞세우는 건 내용이 아니라 **감사 기록**입니다. 어떤 게이트를 통과했는지, 몇 명의 리뷰어가 초안을 공격했고 무엇을 잡았는지, Act 재진입마다의 사유, 계획과 산출 사이의 어긋남. 산출물은 그 아래에 붙습니다. 완성된 문서 자체는 그것이 적대 리뷰어 다섯을 견디고 재진입을 두 번 거쳤다는 걸 보여줄 수 없습니다. 그 기록이 이 페이지를 공유할 값어치로 만듭니다.
