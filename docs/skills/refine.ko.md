...
> 개선하는 손. 벤치마크(리뷰)가 남긴 소견을 반영하고, 목표가 나올 때까지 반복합니다. Check 없는 수정은 신의 손이 아닙니다.

## 빠른 예시

```
/scc:refine "이 아티클을 4.5/5로 올려" --file draft.md
```
...
|--------|-----|--------|
| `--file` | 다듬을 초안 경로 | 필수 |
| `--review` | 초기 리뷰 리포트 경로 (첫 리뷰 사이클 생략) | 없음 |
| `--max` | `1-10` | `3` |
...
- **점수가 안 오른다** -- 한 라운드가 점수를 전혀 못 올리면 refine이 먼저 멈춥니다. 오류가 아니라 수확 체감 처리입니다. 시작 점수가 이미 높으면 올릴 여지 자체가 적습니다.
- **리뷰가 매번 같은 소리를 한다** -- refine이 실제 서브에이전트를 띄우는지 확인하십시오. 한 컨텍스트 안에서 모든 관점을 흉내 내면 집단사고가 생기고 피드백이 반복됩니다.
- **갑자기 되돌려졌다** -- 점수가 떨어진 라운드는 `git checkout -- <file>`로 되돌립니다. 대상 파일이 git 저장소 안에 있고 현재 버전이 커밋돼 있어야 합니다.
- **전제 조건: git 저장소** -- 위 되돌리기 때문에 작업 디렉터리가 git 저장소여야 하고, 대상 파일이 refine 실행 전에 커밋돼 있어야 합니다.
