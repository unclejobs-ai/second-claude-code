...
> 분해하는 손. 서로 의존하지 않는 단위로 나눈 뒤, 각각을 별도 워크트리에서 돌립니다.

**자동 라우팅되지 않습니다.** 독립적인 병렬 단위입니다. 순차 작업은 PDCA 또는 명시적 워크플로입니다. 스킬에 `disable-model-invocation: true`가 붙어 있어 `/scc:batch`로 직접 부를 때만 돕니다. 파일은 플러그인에 그대로 남습니다.
...
**동작 방식:** 스킬이 Explore 에이전트로 범위를 파악한 뒤 2~10개의 독립 단위로
분해합니다. 계획표를 먼저 보여주고 명시적 승인을 기다립니다. 승인된 단위는
격리 워크트리에서 실행되며 동시성은 `--parallel`(기본 3)로 제한됩니다. 상태를
추적하고 완료 후 `.captures/batch-{run_id}/00-summary.md`에 요약합니다.
...
```mermaid
graph TD
    A[분석: 작업 범위 파악, 단위 유형 결정] --> B[분해: 2-10개 독립 단위]
...
- 기본 최대치인 10개보다 많은 단위가 필요하면 배치를 나누거나, 순차 의존성이 있다면 `workflow`를 사용합니다. 현재 `--units` 상한은 10개입니다.
