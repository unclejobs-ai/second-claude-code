...
- **CLI가 없어 검색 범위가 줄었다** -- 외부 검색에 `npx`, `npm`, `gh`를 씁니다. 없으면 조용히 축소 동작하지만 훑는 소스가 줄어듭니다. 전 범위를 쓰려면 설치하십시오.
- **설치 명령에 버전이 안 붙었다** -- 설치는 버전 고정을 요구합니다(예: `claude install user/skill@v1.2.0`). 고정 없는 명령이 나왔다면 실행 전에 버전을 직접 붙이십시오.
