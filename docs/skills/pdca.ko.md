...
> Plan → Do → Check → Act. 하드 게이트. Do 안에서 도메인 스킬. 파일명이 아니라 직무(`researcher`)로 부른다.
...
**진행 과정:**
1. **Plan**: 질문 프로토콜 최대 3개. researcher + analyst + strategist가 브리프를 만든다. 막힌 URL은 `/scc:unblock`.
2. **Plan→Do 게이트**: 브리프 ≥ 3000자, 출처 ≥ 5, 사실 ≥ 8.
3. **Do**: writer가 Plan 아티팩트로 쓴다. 뉴스레터/스레드/쇼츠는 각자 스킬.
4. **Do→Check 게이트**: 길이 바닥, 섹션, Plan 반영.
5. **Check**: 프리셋 리뷰어 병렬. 전원 침묵은 거수기. 코드 리뷰는 설치돼 있으면 coderabbit 우선.
