...
> **하드** 품질 게이트(길이 floor, 리뷰어 모델 다양성, 보정된 5+ 룰), 외부 플러그인 디스패치, 액션 라우터, 포켓몬 테마 conceptual role을 갖춘 Plan → Do → Check → Act 전체 사이클 오케스트레이터.
...
2. **Plan→Do 게이트**: 서로 다른 출처 5개 이상이 포함된 리서치 브리프와 분석 아티팩트를 검증합니다.
