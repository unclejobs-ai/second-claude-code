...
**자동 라우팅되지 않습니다.** 이름 붙은 파이프라인 재실행이며, 두 번째 PDCA가 아닙니다. 게이트 사이클은 계속 PDCA입니다. 스킬에 `disable-model-invocation: true`가 붙어 있어 `/scc:workflow`로 직접 부를 때만 돕니다. 파일은 플러그인에 그대로 남습니다.
...
`/scc:workflow run <preset>`으로 프리셋을 실행합니다:
