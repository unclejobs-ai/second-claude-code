...
**동작 방식:** 스킬이 3단계 순차 실행의 JSON 정의를 생성하고, 각 단계의 출력 선언과 `input_from` 참조의 호환성을 검증한 뒤 파이프라인을 저장합니다. `/second-claude-code:workflow run "weekly-report" --topic "edge computing"`을 실행하면 모든 `{{variable}}` 플레이스홀더를 먼저 해석한 후, 각 단계를 새로운 서브에이전트로 실행하며 데이터를 파일을 통해 전달합니다.
...
- **파이프라인을 찾을 수 없음** -- `/second-claude-code:workflow list`로 파이프라인 이름을 확인하세요. 정의 파일은 `${CLAUDE_PLUGIN_DATA}/pipelines/{name}.json`에 저장됩니다.
