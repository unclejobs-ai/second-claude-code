...
1. **컴패니언 데몬 기반** — 스케줄링, 백그라운드 실행, 알림 라우팅, 세션 리콜 인덱싱을 위한 로컬 데몬 헬퍼와 CLI 진입점을 추가했어요. **다만 run 큐에는 실행기가 없습니다** — `daemon_start_background_run`은 항목만 쓰고 그대로 `queued`로 남습니다. 기반은 있고, 비우는 쪽이 없습니다.
