...
```
second-claude/
├── .claude-plugin/plugin.json    # 플러그인 매니페스트 — MCP 서버: pdca-state, playwright (선택)
├── skills/                       # 11개 스킬 (각각 SKILL.md)
...
`hooks/prompt-detect.mjs` 자동 라우터에는 PDCA 복합 레이어가 있어서 다중 페이즈 의도(예: "알아보고 써줘")를 감지하면 개별 스킬 매칭보다 먼저 `/second-claude-code:pdca`로 라우팅해요.
...
| `UserPromptSubmit` | `prompt-detect.mjs` | 자연어 자동 라우터 (PDCA 복합 의도 감지 포함) |
...
| 서버 | 유형 | 필수 여부 | 역할 |
|------|------|----------|------|
| `pdca-state` | stdio | 필수 | 6개 도구(`get`, `start`, `transition`, `check_gate`, `end`, `update_stuck`)로 PDCA 상태 관리 |
