...
Claude Code **2.1.232**(2026-08-14)부터 호스트가 서브에이전트를 기본 포크한다. SCC는 그 포크를 구현하지 않고 Trajectory도 소유하지 않는다. 리뷰어는 봉투를 남기고, PDCA는 파일을 판정한다. `SendMessage`는 호스트 도구다. 사이클 메모리가 메시지 버스가 아니다.
