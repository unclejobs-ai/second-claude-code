...
tone-guardian: { model: sonnet, role: jigglypuff, purpose: "check voice and audience fit against .data/soul/SOUL.md Tone Rules and Anti-Patterns" }
structure-analyst: { model: sonnet, role: unown, purpose: "check organization, flow, format compliance" }
...
**Tier overrides are upward-only.** `--tier fable` raises the two spine roles — `writer` (smeargle) and `deep-reviewer` (xatu) — for high-stakes runs. Nothing lowers them: not a flag, and not an automatic fallback when a dispatch fails. If those dispatches are exhausted, escalate to the user instead of retrying at a cheaper tier, because every later phase inherits what the writer and the whole-document reviewer missed. To make a run cheaper, cut cycles or reviewers. See `references/design-principles.md` §10.
