...
- **Reviewer independence in the review quorum.** A reviewer-named agent that starts while the active run is in any phase other than Check was borrowed upstream; `subagent-start` records it. At aggregation its report is kept and its vote is not counted, and if exclusion leaves the panel short the consensus is `BLOCKED — QUORUM SHORT`, naming who was barred and how many independent reviewers are missing. Quorum is never reached by relaxing an exclusion. The list clears once consensus is computed, so one upstream loan does not bar an agent forever.
- **Reviewer independence, enforced from the record.** A fork file can carry `participants` — the agents that helped settle it — and `record-verdict` refuses a verdict whose `reviewer` appears in that list. The spec had stated the invariant ("whoever helped write it does not get to pass it") and nothing implemented it. Judged from the participant list, never from a declaration of independence.
