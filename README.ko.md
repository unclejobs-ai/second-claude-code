...
- **테스트 기준선 정리** — 현재 검증 기준은 총 `311`개, `310`개 통과, `1`개 스킵, 실패 `0`개예요
...
**Stage Contracts:** `config/stage-contracts.json`이 code 작업과 content 작업을 구분해서 페이즈별 출구 조건을 다르게 잡아요. 그래서 같은 PDCA라도 코드 리뷰와 보고서 리뷰가 똑같이 판정되지 않아요.
...
`pdca-state` MCP 서버(stdio 방식, `mcp/lib/` 6개 핸들러 모듈로 모듈화)가 세션 간 상태를 관리해요. 전체 표면은 **21개 도구**고, 아래 7개는 코어 PDCA 도구예요:
