...
`pdca-state` MCP 서버(stdio 방식, `mcp/lib/` 6개 핸들러 모듈로 모듈화)가 세션 간 상태를 관리해요. 전체 표면은 **24개 도구**예요.
...
나머지 14개 도구는 soul, project memory, daemon control, session recall, analytics 표면에 걸쳐 있어요.
