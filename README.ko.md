...
- **결정이 세션보다 오래 남습니다.** 답이 둘 이상 방어 가능하면 넘겨짚지 않고 물어보고, 고른 답을 프로젝트에 기준 문서로 씁니다. 고른 방향, 탈락한 안, 각각이 진 이유까지. 다음 세션이 시작 전에 그걸 읽습니다.
...
화요일에 정한 걸 목요일에 새 세션이 뒤집습니다. 이미 탈락시킨 안을 다시 들고 오는데, 디스크에는 탈락시켰다고 적힌 게 없기 때문입니다.

`/scc:coach`는 요청에 방어 가능한 방향이 둘 이상일 때 물어봅니다. 판정 기준은 "이번에 확신이 없는가"가 아니라 "같은 근거를 읽은 다른 유능한 에이전트가 다른 답에 도달할 수 있는가"입니다. 고르면, 그 답을 채팅 로그가 아니라 **당신 프로젝트**에 씁니다.

```
.scc/standards/voice-two-track/STANDARD.md
```

문서에는 고른 방향, 탈락한 안과 각각이 진 이유, 이 결정을 다시 열어야 할 조건, 검증에 쓸 검사가 들어갑니다. 세션은 시작할 때 이걸 읽습니다. 폐기해도 파일은 남고 `superseded`로 바뀝니다. 탈락 이력이 같은 논쟁을 세 번째로 하지 않게 막는 부분이니까요.
...
검사는 코드가 아니라 데이터입니다. 고정 검사기 다섯이 구조화된 인수를 받고, `run:` 같은 자유 문자열 필드가 붙은 기준은 실행하는 대신 거부합니다. 기준 문서는 저장소를 타고 퍼지니까요. 정규식으로 못 가리는 질문은 리뷰어가 답할 때까지 `UNPROVEN`이고, 그 답은 리뷰어가 실제로 읽은 바이트에 묶입니다. 초안을 고치면 다시 unproven으로 돌아갑니다. 채점하는 도구가 합격 도장을 찍는 일은 없습니다.
